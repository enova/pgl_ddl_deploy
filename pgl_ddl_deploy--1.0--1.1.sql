...
ALTER TABLE pgl_ddl_deploy.set_configs ADD CONSTRAINT repset_tables_only_alter_table CHECK ((NOT include_only_repset_tables) OR (include_only_repset_tables AND create_tags = '{"ALTER TABLE"}' AND (drop_tags IS NULL OR drop_tags = '{"DROP TABLE"}')));
...
--This is good.  It would be better though to have an exclusion index if possible.  Could be done via trigger.
ALTER TABLE pgl_ddl_deploy.set_configs ADD CONSTRAINT unique_set_tags UNIQUE (set_name, create_tags, drop_tags);
